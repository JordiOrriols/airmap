...
export default function DraggablePanelContainer({ 
  ...
  const handleDragEnd = (result) => {
    ...
    if (sourceColumn === destColumn) {
      ...
      const items = sourceColumn === 'left' ? [...leftPanels] : [...rightPanels];
      ...
      const sourceItems = sourceColumn === 'left' ? [...leftPanels] : [...rightPanels];
      const destItems = destColumn === 'left' ? [...leftPanels] : [...rightPanels];
            ...
            <Draggable key={panel.id} draggableId={panel.id} index={index}>
              {(provided, snapshot) => (
                <div
                  ...
                  className={`${snapshot.isDragging ? 'opacity-50 scale-95' : ''} transition-all`}
                >
                  {panel.component}
                </div>
              )}
