...
```typescript
import { DEFAULT_CRUISE_SPEED, DISTANCE_UNITS, AIRSPACE_TYPES } from "@/utils/constants";

// Speed validation
const isValidSpeed = (speed: number) => speed >= MIN_CRUISE_SPEED && speed <= MAX_CRUISE_SPEED;
...
// Format distance
const formatDistance = (distance: number) => `${distance.toFixed(1)} ${DISTANCE_UNITS.NM}`;
```
